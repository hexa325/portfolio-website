...
export default function BlueprintExplainer({ imageSrc, alt, hotspots, title }: BlueprintExplainerProps) {
  ...
  return (
    <div className="relative group border-4 border-black dark:border-white bg-zinc-100 dark:bg-zinc-900 overflow-hidden shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] dark:shadow-[12px_12px_0px_0px_rgba(244,244,240,1)] flex flex-col">
      
      {/* HEADER BAR */}
      <div className="bg-black dark:bg-white text-white dark:text-black px-4 py-2 flex justify-between items-center z-10 border-b-4 border-black dark:border-white">
          ...
          <nav className="grid grid-cols-2 lg:grid-cols-1 gap-2">
            {hotspots.map((spot, i) => (
              <button
                ...
                className={`text-left p-2 lg:p-3 font-mono text-[9px] lg:text-[10px] uppercase transition-all flex items-center gap-2 lg:gap-3 border-2 ${
                  activeSpot?.label === spot.label 
                    ? "bg-blue-600 text-white border-blue-600 lg:translate-x-1" 
        ...
        <div className="hidden lg:block relative flex-grow bg-zinc-100 dark:bg-zinc-900 overflow-hidden">
          <div className="absolute inset-0">
            ...
            {imageSrc ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center text-black/5 dark:text-white/5 font-bold text-4xl lg:text-8xl uppercase tracking-tighter select-none">
                {title}
              </div>
            )}

            {/* BLUEPRINT OVERLAY */}
            <div className="absolute inset-0 blueprint-grid opacity-20 lg:opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
            
            {/* SCANLINE EFFECT */}
            <div className="absolute inset-0 pointer-events-none opacity-10 lg:opacity-0 group-hover:opacity-20 transition-opacity overflow-hidden">
          ...
          {activeSpot && (
            <div className="absolute top-4 right-4 bottom-4 w-72 bg-black/95 dark:bg-white/95 text-white dark:text-black p-6 border-l-4 border-blue-600 animate-reveal-up z-30 hidden lg:flex flex-col overflow-hidden">
      ...
      <div className="bg-zinc-300 dark:bg-zinc-700 px-4 py-2 flex justify-between items-center font-mono text-[8px] opacity-60 uppercase border-t-4 border-black dark:border-white">
        <div className="flex gap-4">
          <span className="hidden sm:inline">Sector: 07-G</span>
          <span>Status: {activeSpot ? `LOCK_${activeSpot.label.toUpperCase()}` : "SCANNING..."}</span>
        </div>
