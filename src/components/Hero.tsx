export default function Hero() {
  const ShatterText = ({ children, className }: { children: string; className?: string }) => (
    <div className={`shatter-container ${className}`}>
      {/* Base text that becomes visible after shatter and carries the glitch */}
      <span className="shatter-base block">{children}</span>
      {/* Animated Shards */}
      <span className="shatter-layer shard-top block" aria-hidden="true">{children}</span>
      <span className="shatter-layer shard-mid block" aria-hidden="true">{children}</span>
      <span className="shatter-layer shard-bottom block" aria-hidden="true">{children}</span>
    </div>
  );
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <div className="relative mb-6 md:mb-10 isolate w-full">
            <h1 className="text-[12vw] md:text-[10vw] font-bold leading-[0.8] tracking-tighter select-none">
              {/* Desktop: Kinetic Shatter Effect | Mobile: High-performance standard entry */}
              <div className="hidden md:block">
                <ShatterText className="text-black dark:text-white">SOFTWARE</ShatterText>
                <br />
                <ShatterText className="text-black/20 dark:text-white/20">ENGINEER</ShatterText>
              </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6">
            <div className="overflow-hidden rounded-full">
              <a
                href="#projects"
                className="group relative block px-12 py-6 bg-black text-white dark:bg-white dark:text-black rounded-full overflow-hidden font-bold text-xs uppercase tracking-widest transition-all duration-300 md:hover:scale-105 active:scale-95 animate-reveal-up [animation-delay:2.2s]"
              >
            ...
            <div className="overflow-hidden rounded-full">
              <a
                ...
                className="block px-12 py-6 border border-black/10 dark:border-white/10 rounded-full md:hover:bg-black md:hover:text-white transition-all duration-500 font-bold text-xs uppercase tracking-widest text-black dark:text-white active:scale-95 animate-reveal-up [animation-delay:2.3s]"
              >
                Get Resume
              </a>
            </div>
